...
class Transaction : # 거래 내역을 나타내는 클래스
    def __init__(self, transaction_type, amount, balance_after, description=""):
        self.transaction_id = self.generate_id() # 랜덤 생성
    ...
    @staticmethod   # 클래스, 인스턴스 참조하지 않음
    def generate_id() :
        timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
        random_str = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))
        return f'TXN{timestamp}{random_str}'    # timestamp값과 랜덤 문자열을 합쳐 중복되지 않는 값 생성
    
    def __str__(self):
        return (f'[{self.timestamp.strftime('%Y-%m-%d %H:%M:%S')}] '
                f'{self.type} : {self.amount:,}원 (잔액 : {self.balance_after:,}원')
    
    def to_dict(self) : # 정보를 Json형태로 변환
        return {
...
class Account : 
    ...
    def __init__(self, account_number, owner_name, initial_balance=0) :
        ...
        account_type = self.__class__.__name__  # 클래스 이름을 가져오는 파이썬의 특별한 기능
        Account.accounts_types[account_type] = Account.accounts_types.get(account_type, 0) + 1
            # accounts_type 별로 몇 번 등장했는지 누적 카운트

        # 초기 입금액이 0보다 많으면 거래 기록 추가
        if initial_balance > 0 : 
            self.__transactions.append(
                Transaction("deposit", initial_balance, initial_balance, "계좌 개설 초기 입금")
            )
    ...
    def transaction_count(self) :
        return len(self.__transactions) # transactions 갯수
    ...
    def deposit(self, amount) :
        ...
        if amount <= 0 :
            raise ValueError("입금액은 0원 보다 커야합니다.")
    ...
    def withdraw(self, amount) :
        ...
        if amount <= 0 :
            raise ValueError("출금액은 0원 보다 커야합니다.")
        ...
        return True, f"{amount:,}원이 출금되었습니다. (현재 잔액 : {self.__balance:,}원)"
    

    def transfer(self, target_account, amount) :
        if not isinstance(target_account, Account) : # target_account가 Account로 만든 인스턴스인지 확인
    ...
    def get_transactions(self, limit = None, transaction_type = None) : 
        ...
        transactions.reverse()  # 순서를 바꿔 최신순으로 조회

        if limit :  # limit 갯수만큼만 보고 싶을 때
    ...
    def close_account(self) : 
        ...
        account_type = self.__class__.__name__
        Account.accounts_types[account_type] -= 1
    ...
    def get_bank_statistics(cls) :
        return{
            ...
            'accounts_types' : dict(cls.accounts_types)
        }
    
    @classmethod
    def generate_account_number(cls) :  # 타임스탬프와 랜덤 문자열 합쳐서 계좌번호 생성
...
class SavingsAccount(Account) :
    ...
    def add_monthly_interest(self) :
        ...
        montly_rate = self.interest_rate / 12 / 100
        interest = int(self.balance * montly_rate)

        if interest > 0 :
            self._Account__balance += interest
            Account.total_balance += interest

            transaction = Transaction("interest", interest, self.balance, f"월 이자(연 {self.interest_rate}%)")
...
def demo_bank_system() :
    ...
    print(f"{savings1.owner_name}님의 거래 내역 : ")
    for transaction in savings1.get_transactions(limit=5) :
        print(f"    {transaction}")

    print(f"{savings2.owner_name}님의 거래 내역 : ")
    for transaction in savings2.get_transactions(limit=5) :
        print(f"    {transaction}")
    ...
    print(f"계좌 유형별: {stats['accounts_types']}")
